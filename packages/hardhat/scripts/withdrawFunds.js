...
//3) run: npx hardhat run scripts/withdraw.js --network MUMBAI
async function main() {
    ...
    //call SOBR flow send lump sum method from signers[0]
    await sobrFlow
        .connect(signers[0])
        .withdrawFunds(daix.address, ethers.utils.parseEther("500"))
        .then(function (tx) {
            console.log(`
        Congrats! You've just successfully withdrew funds from the SOBR flow contract.
        Tx Hash: ${tx.hash}
    `)
        })
}
